...
const WORD_SOURCE = {
  animals: `
    ...
    ФЛА-МИН-ГО/FLA-MIN-GO 🦩 | ПИН-ГВИН/PIN-GU-ÏN 🐧 | ТЮ-ЛЕН/ZEE-HOND 🦭 | КИТ/WAL-VIS 🐳
  ...
  food: `
    ...
    КРАС-ТА-ВИ-ЦА/KOM-KOM-MER 🥒 | ЧУШ-КА/PA-PRI-KA 🫑 | ЦА-РЕ-ВИ-ЦА/MAÏS 🌽 | КАР-ТОФ/AARD-AP-PEL 🥔
...
const LEVELS = [
  ...
  { id:6,  minLen:5, maxLen:6,  maxDifficulty:2, wordsToPass:8,  modes:["build","build","missing","syllables","first","listen","read"] },
  { id:7,  minLen:6, maxLen:6,  maxDifficulty:2, wordsToPass:8,  modes:["build","build","missing","syllables","first","listen","read"] },
...
const Play = {
  ...
  nextRound(tutorial){
    ...
    if(tutorial){
      ...
      setTimeout(() => this.speakWord(), 900);
    } else if(mode.showsPicture && (State.progress.autoSpeak || item.audioOnly)){
      // По подразбиране мълчим: картинката вече казва коя е думата, а
      // детето може да я чуе с докосване. Режимът "чуй думата" е изключение
      // и се грижи сам за изговора.
  ...
  buildPicture(word){
    ...
    card.addEventListener("click", () => this.speakWord());
    this.picCard = card;
    return card;
  },
